...
export function GameLobbyPage() {
  ...
  const { gamingList, loading: gamesLoading } = useGames();
  ...
  // 获取游戏列表（使用旧接口数据，类似百家乐）
  useEffect(() => {
    setLoading(gamesLoading);
    
    if (!gamesLoading && gamingList.length > 0) {
      // 根据选中的标签筛选游戏
      const filteredGames = getFilteredGames(activeTab, gamingList);
      
      setAllGamesData(filteredGames);
      setDisplayedGamesCount(12); // 重置显示数量
      
      // 使用 requestAnimationFrame 确保在下一帧更新，减少抖动
  ...
  }, [activeTab, gamingList, gamesLoading]);
