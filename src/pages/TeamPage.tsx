    ...
    try {
      ...
      try {
        ...
        if (linkRes.code === 200) {
          ...
          console.log('📋 解析后的链接列表:', links);
          
          if (links.length > 0) {
            const firstLink = links[0];
            console.log('✅ 使用现有邀请链接:', firstLink);
            setInviteLink({
              id: firstLink.id || 0,
            ...
            // 如果没有链接，创建一个（需要传递rates参数）
            console.log('➕ 没有找到邀请链接，创建新的，使用rates:', agentRates);
            // 确保rates不为空
            const ratesToSend = agentRates || [];
            // 添加 created_at 字段（当前时间）
            const createRes = await createInviteLink({
              rates: ratesToSend,
              created_at: new Date().toISOString().replace('T', ' ').substring(0, 19)
            });
            console.log('➕ 创建邀请链接响应:', createRes);
            
            if (createRes.code === 200) {
              // 处理不同的响应结构
              const newLink = createRes.data?.data || createRes.data || createRes;
              console.log('✅ 创建成功，新链接数据:', newLink);
              ...
              if (newLink) {
                ...
                console.error('❌ 创建成功但数据为空');
                // 如果创建成功但没有返回数据，尝试从用户信息获取邀请码
                if (userInfo?.invite_code) {
                  console.log('📝 使用用户信息中的邀请码:', userInfo.invite_code);
                  setInviteLink({
                    id: 0,
                    code: userInfo.invite_code,
                    url: '',
              ...
              console.error('❌ 创建邀请链接失败:', createRes);
              // 如果创建失败，尝试使用用户信息中的邀请码
              if (userInfo?.invite_code) {
                console.log('📝 创建失败，使用用户信息中的邀请码:', userInfo.invite_code);
                setInviteLink({
                  id: 0,
                  code: userInfo.invite_code,
                  url: '',
          ...
          console.error('❌ 获取邀请链接列表失败:', linkRes);
          // 如果获取失败，尝试使用用户信息中的邀请码
          if (userInfo?.invite_code) {
            console.log('📝 获取失败，使用用户信息中的邀请码:', userInfo.invite_code);
            setInviteLink({
              id: 0,
              code: userInfo.invite_code,
              url: '',
        ...
        // 如果出现异常，尝试使用用户信息中的邀请码
        if (userInfo?.invite_code) {
          console.log('📝 异常情况，使用用户信息中的邀请码:', userInfo.invite_code);
          setInviteLink({
            id: 0,
            code: userInfo.invite_code,
            url: '',
