      ...
      if (paymentMode === 'company') {
        ...
        if (paymentChannel === 'alipay') {
          ...
          matchedOnlinePayment = onlinePaymentList.find(p => {
            // 精确匹配
            if (p.type === 'online_alipay') return true;
            // 包含匹配
            if (p.type && p.type.includes('online_') && p.type.includes('alipay')) return true;
            // 类型文本匹配
            if (p.type_text && (p.type_text.includes('支付宝') || p.type_text.includes('alipay') || p.type_text.includes('Alipay'))) return true;
            // 描述匹配
            if (p.desc && (p.desc.includes('支付宝') || p.desc.includes('alipay') || p.desc.includes('Alipay'))) return true;
            return false;
          }) || null;
        } else if (paymentChannel === 'wechat') {
          matchedOnlinePayment = onlinePaymentList.find(p => {
            // 精确匹配
            if (p.type === 'online_wxpay' || p.type === 'online_wechat') return true;
            // 包含匹配
            if (p.type && p.type.includes('online_') && (p.type.includes('wx') || p.type.includes('wechat'))) return true;
            // 类型文本匹配
            if (p.type_text && (p.type_text.includes('微信') || p.type_text.includes('wechat') || p.type_text.includes('WeChat') || p.type_text.includes('wx'))) return true;
            // 描述匹配
            if (p.desc && (p.desc.includes('微信') || p.desc.includes('wechat') || p.desc.includes('WeChat') || p.desc.includes('wx'))) return true;
            return false;
          }) || null;
        } else if (paymentChannel === 'unionpay') {
          // 银联支付匹配
          matchedOnlinePayment = onlinePaymentList.find(p => {
            // 精确匹配
            if (p.type === 'online_unionpay' || p.type === 'online_yl') return true;
            // 包含匹配
            if (p.type && p.type.includes('online_') && (p.type.includes('unionpay') || p.type.includes('yl'))) return true;
            // 类型文本匹配
            if (p.type_text && (p.type_text.includes('银联') || p.type_text.includes('unionpay') || p.type_text.includes('UnionPay'))) return true;
            // 描述匹配
            if (p.desc && (p.desc.includes('银联') || p.desc.includes('unionpay') || p.desc.includes('UnionPay'))) return true;
            return false;
          }) || null;
        }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#151A23', color: '#fff', paddingBottom: '80px', touchAction: 'pan-y' }}>
      ...
      <div style={{ padding: '16px' }}>
        ...
        {/* 存款方式选择 */}
        <div style={{ marginBottom: '20px' }}>
          <div style={{ fontSize: '14px', fontWeight: 500, marginBottom: '12px', color: '#fff' }}>存款方式</div>
          {depositCurrency === 'usdt' ? (
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        padding: '6px',
                        borderRadius: '6px',
                        backgroundImage: 'url(https://www.xpj00000.vip/indexImg/default-sports-mini.BJ4yNOA9.svg)',
                        backgroundSize: 'cover',
                        backgroundPosition: 'center',
                        backgroundRepeat: 'no-repeat',
                        backgroundColor: 'transparent',
              ...
              {onlinePaymentList.filter(payment => payment.is_open !== false).length === 0 && (
                <div style={{ 
                  width: '100%', 
                  padding: '20px', 
                  textAlign: 'center', 
